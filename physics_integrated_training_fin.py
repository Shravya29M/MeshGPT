...
def train_with_taichi_pointcloud_metrics(
    ...
    quality_eval_freq=2,  # Can evaluate more often
):
    ...
    for epoch in range(1, num_epochs+1):  #Training code
        model.train()
        train_loss = 0.0
        train_chamfer = 0.0
        ...
        model.eval()
        val_loss = 0.0                            #Validating on 10% of data
        ...
        if (epoch % quality_eval_freq==0):
            ...
            with torch.no_grad():
                ...
                train_quality_metrics=evaluate_batch_quality_taichi(train_recon) #Quality check on train set
                val_batch=next(iter(val_loader)).to(device)
                val_recon, _ =model(val_batch)
                val_quality_metrics=evaluate_batch_quality_taichi(val_recon)#Quality chcecl on Valid set
